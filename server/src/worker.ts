import { executionWorker } from "./queue/executionWorker";
...
const shutdown=async(signal:string)=>{clearInterval(timer);console.log("[vertice-worker] "+signal);await executionWorker.close();process.exit(0);};
