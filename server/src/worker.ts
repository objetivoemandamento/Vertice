...
const shutdown=async(signal:string)=>{clearInterval(timer);console.log("[vertice-worker] "+signal);const {executionWorker}=await import("./queue/executionWorker");await executionWorker.close();process.exit(0);};
