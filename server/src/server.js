...
async function registerDeviceNow({tenantId,userId,deviceId,mode,deviceName}) {
...
app.post('/devices/register', auth, async (req, res) => {
  ...
  try{const result=await registerDeviceNow({tenantId:req.user.tenant_id,userId:req.user.sub,deviceId:id,mode,deviceName:name});return res.status(200).json({ok:true,...result});}catch{return errorJson(res,403,'Registro de dispositivo rejeitado.','POLICY_DENIED');}
});
  ...
  if (mode === 'operacao' && aiResult.shouldExecute && !emergencyStopped) {
    try {
      ...
      if (req.user.role === 'OWNER') {
        await registerDeviceNow({ tenantId, userId: actorUserId, deviceId, mode: 'operacao', deviceName: String(req.body?.deviceName || 'Android • OPERAÇÃO').slice(0, 100) });
      } else {
