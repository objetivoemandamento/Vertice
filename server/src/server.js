...
app.post('/public/signup/checkout',async(req,res)=>{
  ...
  try{
    ...
    if(market.countryCode==='BR'||market.countryCode==='MX'){
      ...
      return res.status(201).json({paymentId:payment.id,provider:'mercado_pago',plan:selected.id,amount,currency:currencyCode,checkoutUrl,statusToken:issuePaymentCapability(String(payment.id),String(u.id))});
    }
