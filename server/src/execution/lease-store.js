...
function expired(execution, now = Date.now()) {
  return !execution || !Number.isFinite(execution.expiresAt) || now > execution.expiresAt;
}
...
function canFinalize(execution, leaseId) {
  return Boolean(execution && leaseId && execution.leaseId === leaseId && !expired(execution));
}
