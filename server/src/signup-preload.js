...
function signupToken(saleId) {
  return crypto.createHmac('sha256', jwtSecret()).update(String(saleId)).digest('hex');
}
function validSignupToken(saleId, provided) {
  const a = Buffer.from(signupToken(saleId));
...
async function mp(path, options = {}) {
  ...
  try {
    ...
    if (!r.ok) {
      const e = new Error(`Mercado Pago: ${data?.message || data?.error || `HTTP ${r.status}`}`);
      e.status = r.status;
      throw e;
    }
    return data;
...
async function syncSignup(sale) {
  ...
  db.prepare('UPDATE sales SET status=?, mp_payment_id=COALESCE(?,mp_payment_id), paid_at=CASE WHEN ?="paid" THEN COALESCE(paid_at,?) ELSE paid_at END WHERE id=?')
    .run(status, Array.isArray(order?.transactions?.payments) ? String(order.transactions.payments.find(p => ['approved','processed','accredited'].includes(String(p?.status || '').toLowerCase()))?.id || '') || null : null, status, now, sale.id);
  if (status === 'paid') {
    const sub = db.prepare('SELECT id FROM subscriptions WHERE customer_id=? ORDER BY updated_at DESC LIMIT 1').get(sale.customer_id);
    const end = addMonth();
    if (sub) db.prepare('UPDATE subscriptions SET plan=?,status="active",current_period_end=?,updated_at=? WHERE id=?').run(sale.plan || 'basic', end, now, sub.id);
    else db.prepare('INSERT INTO subscriptions VALUES(?,?,?,?,?,?)').run(crypto.randomUUID(), sale.customer_id, sale.plan || 'basic', 'active', end, now);
  }
  return status;
}
...
function install(app) {
  ...
  app.get('/public/plans', (req, res) => {
    res.json({ plans: plans().map(p => ({ id: p.id, name: p.name, price: p.price, description: p.description })) });
  });
  ...
  app.post('/public/signup/checkout', async (req, res) => {
    ...
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email) || password.length < 8 || password.length > 128 || !selected) {
      return jsonError(res, 400, 'Informe e-mail válido, senha entre 8 e 128 caracteres e um plano válido.');
    }
    if (!process.env.MERCADOPAGO_ACCESS_TOKEN) return jsonError(res, 503, 'Mercado Pago não configurado no servidor.');
    try {
      let customer = db.prepare('SELECT id,password_hash FROM customers WHERE email=?').get(email);
      const now = new Date().toISOString();
      if (customer) {
      ...
      const externalReference = `VTX-SIGNUP-${customer.id.replace(/-/g, '')}-${saleId.replace(/-/g, '')}`;
      db.prepare('INSERT INTO sales(id,customer_id,amount,product,channel,status,created_at,external_reference) VALUES(?,?,?,?,?,?,?,?)')
        .run(saleId, customer.id, selected.price, `VÉRTICE — ${selected.name}`, 'mercado_pago', 'pending', now, externalReference);
      const order = await mp('/v1/orders', {
        ...
        body: JSON.stringify({
          ...
          items: [{ title: `VÉRTICE — ${selected.name}`, description: selected.description, quantity: 1, unit_price: selected.price.toFixed(2) }]
        })
      });
      const orderId = String(order.id || '');
      const checkoutUrl = String(order.checkout_url || '');
      if (!orderId || !checkoutUrl) throw new Error('Mercado Pago não retornou o checkout.');
      ...
      return res.status(e.status && e.status < 500 ? e.status : 502).json({ error: e.message || 'Não foi possível iniciar o pagamento.' });
    }
  });

  app.get('/public/signup/status', async (req, res) => {
    const signupId = String(req.query?.signupId || '').trim();
    ...
    const sale = db.prepare('SELECT * FROM sales WHERE id=?').get(signupId);
    if (!sale || !sale.mp_order_id) return jsonError(res, 404, 'Pagamento não encontrado.');
    try {
      const status = await syncSignup(sale);
      if (status !== 'paid') return res.json({ status });
      const customer = db.prepare('SELECT id,email FROM customers WHERE id=?').get(sale.customer_id);
      const token = jwt.sign({ sub: customer.id, email: customer.email, role: 'CUSTOMER' }, jwtSecret(), { expiresIn: '30d' });
      return res.json({ status: 'paid', token, customer: { id: customer.id, email: customer.email }, plan: sale.plan || 'basic' });
