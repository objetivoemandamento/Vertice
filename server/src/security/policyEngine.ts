...
export function evaluatePolicy(input:ActionIntent):PolicyDecision{
 ...
 if(input.resource==="command"&&input.operation==="create"){const mode=String(input.payload?.mode||"");const actorRole=String(input.payload?.actorRole||"");if(mode==="operacao"&&actorRole==="OWNER")return{risk:"automatic",allowed:true,reason:"Comando operacional solicitado explicitamente pelo OWNER.",policyVersion:VERSION};return mode==="operacao"?{risk:"mfa",allowed:true,reason:"Comando operacional exige MFA.",policyVersion:VERSION}:{risk:"automatic",allowed:true,reason:"Comando não operacional permitido automaticamente.",policyVersion:VERSION};}
