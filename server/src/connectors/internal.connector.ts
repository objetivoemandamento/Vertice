...
export const internalConnector:Connector={name:"internal",supports:i=>i.resource==="device"||i.resource==="command",execute:async(i:ActionIntent)=>{
 ...
 if(i.resource==="device"){if(!x.deviceId||!x.mode)throw new Error("DEVICE_FIELDS_REQUIRED");await withTransaction(async c=>{await c.query("insert into devices(id,user_id,tenant_id,device_name,mode,last_seen_at) values($1,$2,$3,$4,$5,now()) on conflict(id) do update set user_id=excluded.user_id,tenant_id=excluded.tenant_id,device_name=excluded.device_name,mode=excluded.mode,last_seen_at=now()",[x.deviceId,x.userId,i.tenantId,x.deviceName||"",x.mode])});return {deviceId:x.deviceId,mode:x.mode};}
