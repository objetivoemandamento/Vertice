...
export const internalConnector:Connector={name:"internal",supports:i=>i.resource==="device"||i.resource==="command",execute:async(i:ActionIntent)=>{
 ...
 if(i.operation==="create"){await withTransaction(async c=>{const d=await c.query("select user_id,mode from devices where id=$1",[x.deviceId]);if(!d.rows[0]||d.rows[0].user_id!==x.userId)throw new Error("DEVICE_NOT_OWNED");if(x.mode==="operacao"&&d.rows[0].mode!=="operacao")throw new Error("DEVICE_MODE_MISMATCH");await c.query("insert into commands(id,user_id,device_id,mode,command,status) values($1,$2,$3,$4,$5,'queued')",[x.commandId,x.userId,x.deviceId,x.mode,x.command])});return {commandId:x.commandId,status:"queued"};}
 if(i.operation==="update"){if(!x.status)throw new Error("COMMAND_STATUS_REQUIRED");const r=await import("../db").then(m=>m.query("update commands set status=$1,updated_at=now() where id=$2 and user_id=$3 and device_id=$4 and status='running' returning status",[x.status,x.commandId,x.userId,x.deviceId]));if(!r.rows[0])throw new Error("COMMAND_NOT_FOUND");return {commandId:x.commandId,status:r.rows[0].status};}
