...
@Composable private fun VerticeApp(){
...
@Composable private fun ModeScreen(api:VerticeApi,token:String,owner:Boolean,session:VerticeSession,onSelected:(String)->Unit){val scope=rememberCoroutineScope();var busy by remember{mutableStateOf<String?>(null)};var error by remember{mutableStateOf("")};Column(Modifier.fillMaxSize().padding(22.dp),verticalArrangement=Arrangement.Center){Text("ESCOLHA O MODO",fontSize=27.sp,fontWeight=FontWeight.Bold);Spacer(Modifier.height(16.dp));ModeCard("COMANDO","Decisão, estratégia e conversa",busy=="comando"){busy="comando";scope.launch{val r=withContext(Dispatchers.IO){api.registerDevice(token,session.deviceId,"comando","Android • COMANDO",owner)};r.onSuccess{onSelected("comando")}.onFailure{error=it.message?:"Falha"};busy=null}};Spacer(Modifier.height(10.dp));ModeCard("OPERAÇÃO","Execução real no Android",busy=="operacao"){busy="operacao";scope.launch{val r=withContext(Dispatchers.IO){api.registerDevice(token,session.deviceId,"operacao","Android • OPERAÇÃO",owner)};r.onSuccess{onSelected("operacao")}.onFailure{error=it.message?:"Falha"};busy=null}};Spacer(Modifier.height(10.dp));ModeCard("MONITORAMENTO","Acompanhamento e indicadores",busy=="monitoramento"){busy="monitoramento";scope.launch{val r=withContext(Dispatchers.IO){api.registerDevice(token,session.deviceId,"monitoramento","Android • MONITORAMENTO",owner)};r.onSuccess{onSelected("monitoramento")}.onFailure{error=it.message?:"Falha"};busy=null}};if(error.isNotBlank())Text(error,color=MaterialTheme.colorScheme.error)}}
...
@Composable private fun CommandScreen(api:VerticeApi,token:String,role:String,mode:String,session:VerticeSession,onModeChange:(String)->Unit,onLogout:()->Unit){val scope=rememberCoroutineScope();val list=rememberLazyListState();var input by remember{mutableStateOf("")};var chat by remember{mutableStateOf(listOf(ChatLine(false,"VÉRTICE ativo. Diga o que precisa decidir, pesquisar, estruturar ou executar.")))};var busy by remember{mutableStateOf(false)};var lastId by remember{mutableStateOf<String?>(null)};var status by remember{mutableStateOf("")};LaunchedEffect(mode){withContext(Dispatchers.IO){api.registerDevice(token,session.deviceId,mode,"Android • ${mode.uppercase()}",role=="OWNER")}};LaunchedEffect(chat.size){if(chat.isNotEmpty())list.animateScrollToItem(chat.lastIndex)};LaunchedEffect(lastId){val id=lastId?:return@LaunchedEffect;repeat(30){delay(1000);withContext(Dispatchers.IO){api.commands(token)}.onSuccess{items->items.firstOrNull{it.id==id}?.let{x->status="Status: ${x.status}${if(x.message.isNotBlank())" • ${x.message}" else ""}"}}}};Column(Modifier.fillMaxSize().padding(16.dp)){Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.SpaceBetween){Column(Modifier.weight(1f)){Text("VÉRTICE",fontSize=27.sp,fontWeight=FontWeight.Bold);Text("${role} • ${mode.uppercase()}",fontSize=12.sp)};TextButton(onClick=onLogout){Text("Sair")}};Spacer(Modifier.height(8.dp));Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(5.dp)){MODES.forEach{m->if(m==mode)Button(onClick={}){Text(m.uppercase())}else OutlinedButton(onClick={onModeChange(m)}){Text(m.uppercase())}}};Spacer(Modifier.height(8.dp));LazyColumn(Modifier.weight(1f),state=list,verticalArrangement=Arrangement.spacedBy(7.dp)){items(chat){line->Card(Modifier.fillMaxWidth()){Column(Modifier.padding(10.dp)){Text(if(line.fromUser)"VOCÊ" else "VÉRTICE",fontSize=11.sp,fontWeight=FontWeight.Bold);Text(line.text)}}}};OutlinedTextField(input,{input=it},Modifier.fillMaxWidth(),label={Text("Fale com o VÉRTICE")},maxLines=4);Spacer(Modifier.height(6.dp));Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(8.dp)){Button(onClick={val text=input.trim();if(text.isBlank())return@Button;input="";chat=chat+ChatLine(true,text);busy=true;scope.launch{val reply=withContext(Dispatchers.IO){api.chat(token,text,mode)};reply.onSuccess{chat=chat+ChatLine(false,it.answer)}.onFailure{chat=chat+ChatLine(false,it.message?:"Falha")};if(mode=="operacao"&&reply.isSuccess){val cmd=withContext(Dispatchers.IO){api.sendCommand(token,text,mode,session.deviceId,owner=role=="OWNER")};cmd.onSuccess{lastId=it.id;status="Execução iniciada • ${it.status}"}.onFailure{status=it.message?:"Falha ao enfileirar"}};busy=false}},Modifier.weight(1f),enabled=!busy){Text(if(busy)"ENVIANDO…" else "ENVIAR")};OutlinedButton(onClick={chat=listOf(ChatLine(false,"Conversa reiniciada."));lastId=null;status=""},enabled=!busy){Text("LIMPAR")}};if(status.isNotBlank()){Spacer(Modifier.height(6.dp));Text(status,fontSize=12.sp)}}}
