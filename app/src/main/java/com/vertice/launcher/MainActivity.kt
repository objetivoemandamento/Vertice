...
private fun VerticeApp() {
    ...
    var permissionReady by remember {
        mutableStateOf(mode != "operacao" || accessibilityEnabled(context))
    }
    ...
    MaterialTheme(colorScheme = darkColorScheme()) {
        Surface(Modifier.fillMaxSize()) {
            when {
                ...
                !permissionReady -> PermissionSetupScreen(context) {
                    permissionReady = mode != "operacao" || accessibilityEnabled(context)
                }
                mode.isNullOrBlank() -> ModeSelectionScreen(
                    ...
                    mode = selected
                    permissionReady = selected != "operacao" || accessibilityEnabled(context)
                }
                else -> CommandCenter(
                    api = api,
                    token = token!!,
...
private fun PermissionSetupScreen(context: Context, onRefresh: () -> Unit) {
    ...
    Column(
        ...
        Text("O VÉRTICE precisa das permissões abaixo para operar neste aparelho.")
        Spacer(Modifier.height(16.dp))
        PermissionCard("Notificações", notificationGranted) {
        ...
        PermissionCard("OPERAÇÃO — Acessibilidade", accessibility) {
            openAccessibilitySettings(context)
        }
        Spacer(Modifier.height(14.dp))
        Text(
            if (accessibility) "✅ Acessibilidade pronta."
...
@Composable
private fun PermissionCard(title: String, ready: Boolean, onClick: () -> Unit) {
    Card(Modifier.fillMaxWidth()) {
        Row(
            ...
            Column(Modifier.weight(1f)) {
                ...
                Text(if (ready) "Pronta" else "Pendente", fontSize = 12.sp)
            }
            OutlinedButton(onClick = onClick, enabled = !ready) {
                Text(if (ready) "ATIVA" else "ATIVAR")
            }
        }
    }
}
            ...
            onClick = {
                ...
                scope.launch {
                    ...
                    if (result.isSuccess) {
                        ...
                        error = result.exceptionOrNull()?.message ?: "Falha no login."
                    }
                }
            },
            modifier = Modifier.fillMaxWidth(),
...
private fun ModeSelectionScreen(
    ...
    fun selectMode(selected: String) {
        busy = selected
        error = ""
        scope.launch {
            val result = withContext(Dispatchers.IO) {
            ...
            if (result.isSuccess) onSelected(selected)
            else error = result.exceptionOrNull()?.message ?: "Falha ao registrar o terminal."
            busy = null
        }
    }

    Column(Modifier.fillMaxSize().padding(22.dp), verticalArrangement = Arrangement.Center) {
        Text("ESCOLHA O MODO", fontSize = 27.sp, fontWeight = FontWeight.Bold)
        Spacer(Modifier.height(16.dp))
        ModeCard("COMANDO", "Decisão, estratégia e conversa", busy == "comando") { selectMode("comando") }
        Spacer(Modifier.height(10.dp))
        ModeCard("OPERAÇÃO", "Execução real no Android", busy == "operacao") { selectMode("operacao") }
        Spacer(Modifier.height(10.dp))
        ModeCard("MONITORAMENTO", "Acompanhamento e indicadores", busy == "monitoramento") { selectMode("monitoramento") }
        if (error.isNotBlank()) {
            Spacer(Modifier.height(10.dp))
            Text(error, color = MaterialTheme.colorScheme.error)
        }
...
private fun CommandCenter(
    ...
    var chat by remember {
        mutableStateOf(
            listOf(ChatLine(false, "VÉRTICE ativo. Diga o que precisa decidir, pesquisar, estruturar ou executar."))
        )
    }
    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            Column(Modifier.weight(1f)) {
                ...
                Text("$role • ${mode.uppercase()}", fontSize = 12.sp)
            }
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(5.dp)) {
            MODES.forEach { current ->
                if (current == mode) {
                    Button(onClick = {}) { Text(current.uppercase()) }
                } else {
                    OutlinedButton(onClick = { onModeChange(current) }) { Text(current.uppercase()) }
                }
            }
        }
        Spacer(Modifier.height(8.dp))
            ...
            Button(
                onClick = {
                    ...
                    scope.launch {
                        ...
                        if (reply.isSuccess) {
                            chat = chat + ChatLine(false, reply.getOrThrow().answer)
                            if (mode == "operacao") {
                                val command = withContext(Dispatchers.IO) {
                                    api.sendCommand(token, text, mode, session.deviceId, owner = role == "OWNER")
                                }
                                if (command.isSuccess) {
                                    status = "Execução iniciada • ${command.getOrThrow().status}"
                                } else {
                                    status = command.exceptionOrNull()?.message ?: "Falha ao enfileirar comando."
                                }
                            }
                        } else {
