...
class VerticeApi(private val baseUrl:String){
 ...
 fun chat(token:String,message:String,mode:String):Result<AiReply> = requestResult{val b=JSONObject().put("message",message).put("mode",mode).toString().toRequestBody(json);val q=authorized("/ai/agent/chat",token).post(b).build();http.newCall(q).execute().use{r->requireAuthState(r);if(!r.isSuccessful)error(readError(r.code,r.body?.string()));val o=JSONObject(r.body?.string().orEmpty());val a=o.optJSONArray("actions");val actions=buildList<String>{if(a!=null)for(i in 0 until a.length()){val x=a.opt(i);if(x is JSONObject)add(x.optString("label").ifBlank{x.optString("type")})else add(x.toString())}};AiReply(o.optString("answer"),actions,o.optString("command",""),o.optBoolean("shouldExecute",false))}}
